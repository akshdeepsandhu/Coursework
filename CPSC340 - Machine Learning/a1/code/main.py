...
if __name__ == "__main__":
    ...
    if question == "3.3":
        ...
        col_mean = df.mean()
        print(col_mean == max(mean))
        print(col_mean == min(mean))
        ...
        for i, max_depth in enumerate(depths):
            ...
            #extra code to output best model
            if not lowest_error_model:
                if sklearn_tree_errors[i] == 0:
                    utils.plotClassifier(model, X, y)
                    fname = os.path.join("..", "figs", "q6_6_decsionBoundary.pdf")
                    plt.savefig(fname)
