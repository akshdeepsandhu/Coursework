...
if __name__ == "__main__":
    ...
    if question == "2":
        ...
        l1_fit = LogisticRegression(penalty = 'l1', fit_intercept = False,solver='liblinear').fit(XBin,yBin)
        coef_l1 = l1_fit.coef_[0]
        print("\nTraining error %.3f" % utils.classification_error(l1_fit.predict(XBin),yBin))
        print("Validation error %.3f" % utils.classification_error(l1_fit.predict(XBinValid), yBinValid))
        print("# nonZeros for L1 : %d" % (coef_l1!= 0).sum())

        l2_fit = LogisticRegression(penalty = 'l2', fit_intercept = False,solver='liblinear').fit(XBin,yBin)
        coef_l2 = l2_fit.coef_[0]
        print("\nTraining error %.3f" % utils.classification_error(l2_fit.predict(XBin),yBin))
        print("Validation error %.3f" % utils.classification_error(l2_fit.predict(XBinValid), yBinValid))
        print("# nonZeros for L1 : %d" % (coef_l2!= 0).sum())
