...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for(unsigned x = 0; x< image.width(); x++){
    for(unsigned y = 0; y < image.height(); y++){
      int xdist = (centerX-x);
      int ydist = (centerY-y);
      double deltalum = sqrt(xdist*xdist + ydist*ydist); //qrt(pow((x-centerX),2) + pow((y-centerY)), 2) *0.5;
      HSLAPixel *pixel = image.getPixel(x,y);
      pixel->l = max(0.0, pixel->l*(1-(deltalum*0.005)));
    }
  }
...
PNG ubcify(PNG image) {
   for(unsigned x = 0; x< image.width(); x++){
    for(unsigned y = 0; y < image.height(); y++){
      HSLAPixel *pixel = image.getPixel(x,y);
      double huenum = pixel->h;
      double ydif = abs(huenum - 40);
      double bdif = abs(huenum - 210);
      if(ydif > bdif){
        pixel->h = 210;
      }
      else{
